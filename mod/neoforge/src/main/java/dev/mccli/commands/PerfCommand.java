    ...
    public CompletableFuture<JsonObject> execute(JsonObject params) {
        return MainThreadExecutor.submit(() -> {
            ...
            if (client.level != null && client.levelRenderer != null) {
                // Note: countRenderedChunks() may not be available - use alternative or comment out
                // result.addProperty("chunk_updates", client.levelRenderer.countRenderedChunks());

                if (client.level.getEntityCount() >= 0) {
                    result.addProperty("entity_count", client.level.getEntityCount());
                }
            }
